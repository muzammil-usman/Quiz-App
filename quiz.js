...
function submission(e) {
  e.preventDefault();
  for (var m = 0; m < QuestionArr.length; m++) {
    var elements = document.getElementsByName(`question${m + 1}`);
    var answered = false;
    for (var n = 0; n < elements.length; n++) {
      if (elements[n].checked) {
        answered = true;
        if (elements[n].value === QuestionArr[m].Correctopt) {
          score += 1;
        }
        break;
      }
    ...
    if (!answered) {
      alert(`You Missed Question ${m + 1} Give Answer of it `);
      return;
    }
  }
  if (answered) {
...
function result() {
  ...
  studentName.innerText = "Name : " + userName.username;
  let percentageStatus = document.createElement("h2");
  percentageStatus.innerText = "Percentage : " + percentage + "%";
  let scoreStatus = document.createElement("h2");
  scoreStatus.innerText = "Score : " + score + " /30";
  main.setAttribute("class", "main2");
  modal.appendChild(resultHeading);
  modal.appendChild(studentName);
  modal.appendChild(scoreStatus);
  modal.appendChild(percentageStatus);
